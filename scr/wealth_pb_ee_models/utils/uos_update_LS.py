...
# @author: R-Broadley
# Edited on 04/22/2025 by Luis Sigcha for compatibility with current Python libraries
...
def extract_metadata(header):
    ...
    start_datetime = datetime(int(header[261]) + 2000, int(header[260]), int(header[259]),
                          int(header[256]), int(header[257]), int(header[258]))

    stop_datetime = datetime(int(header[267]) + 2000, int(header[266]), int(header[265]),
                         int(header[262]), int(header[263]), int(header[264]))
    ...
    device_id = ((int(header[10]) % 10) * 100000 + int(header[14]) * 10000 +
                 int(header[40]) * 4096 + int(header[11]) * 256 + int(header[12]) * 16 +
                 int(header[13]))
...
def extract_accelerometer_data(body, firmware, datx):
    ...
    # Change the shape argument from a list to a tuple
    signals = np.zeros((max_rows, 3), dtype=np.uint8) 
